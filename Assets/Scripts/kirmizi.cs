...
public class kirmizi : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_hareket._puan == -1) { _hareket._hiz = 0;_hareket2._hiz = 0; }
        if (_hareket2._puan == -1) { _hareket._hiz = 0; _hareket2._hiz = 0; }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.name == "Capsule")
        {
            ...
            GameObject.Find("Capsule").GetComponent<Transform>().localScale += new Vector3(1f, 0, 1f);
        }
        if (other.gameObject.name == "Capsule_iki")
        {
            ...
            transform.position = new Vector3(Random.Range(-15.5f, 25), 0, Random.Range(-39, 39));
            GameObject.Find("Capsule_iki").GetComponent<Transform>().localScale += new Vector3(0.1f, 0, 0.1f);
        }
    }
}
